...
get_codeberg_key() {
    if [ "${KEY_ID}" == '' ]; then
        read -e -p "Please enter the CodeBerg account:" KEY_ID
        [ "${KEY_ID}" == '' ] && echo -e "${ERROR} Invalid input." && exit 1
    fi
    echo -e "${INFO} The CodeBerg account is: ${KEY_ID}"
    echo -e "${INFO} Get key from CodeBerg..."
    PUB_KEY=$(curl -fsSL https://codeberg.org/${KEY_ID}.keys)
    if [ "${PUB_KEY}" == 'Not Found' ]; then
        echo -e "${ERROR} CodeBerg account not found."
        exit 1
    elif [ "${PUB_KEY}" == '' ]; then
        echo -e "${ERROR} This account ssh key does not exist."
        exit 1
...
while getopts "og:c:u:f:p:d" OPT; do
    case $OPT in
    o)
        OVERWRITE=1
        ;;
